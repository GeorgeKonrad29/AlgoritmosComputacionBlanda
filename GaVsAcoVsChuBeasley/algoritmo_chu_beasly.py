...
def swap_mutation(tour: List[int], p_mut: float = 0.2) -> List[int]:
	"""Aplica mutación por swap. Realiza 1-2 swaps con probabilidad p_mut."""
	t = tour[:]
	n = len(t)
	if n < 2:
		return t
	# Realizar 1-2 swaps en lugar de n * p_mut swaps
	if random.random() < p_mut:
		a, b = random.sample(range(n), 2)
		t[a], t[b] = t[b], t[a]
		# Segundo swap opcional con menor probabilidad
		if random.random() < p_mut * 0.5:
			a, b = random.sample(range(n), 2)
			t[a], t[b] = t[b], t[a]
	return t
...
def initialize_population(coords: List[Tuple[float, float]], pop_size: int, seed: Optional[int] = None, max_tries: int = 50) -> List[List[int]]:
	# No establecer semilla aquí, se establece en la función llamadora
	n = len(coords)
...
def cbga(coords: List[Tuple[float, float]], pop_size: int = 100, generations: int = 500, min_diversity: float = 0.2,
		...
		apply_2opt_on_new: bool = True, tournament_k: int = 3) -> Tuple[List[int], float, List[float]]:
	...
	Returns:
		(best_tour, best_distance, convergence_history)
	"""
	if seed is not None:
		random.seed(seed)
	n = len(coords)
	...
	for gen in range(generations):
		...
		if child_fit < best_dist:
			best_dist = child_fit
			best_tour = child[:]
		# Registrar mejor distancia al final de cada generación
		history.append(best_dist)
	# end generations
	return best_tour, best_dist, history


def nearest_neighbor_multistart(coords: List[Tuple[float, float]], restarts: int = 10, seed: int | None = None) -> Tuple[List[int], float]:
	"""Heurística constructiva: Vecino más cercano + 2-opt con múltiples reinicios."""
	if seed is not None:
		random.seed(seed)
	n = len(coords)
	if n == 0:
...
def main():
	...
	if args.ga:
		...
		tour, dist, _ = cbga(coords, pop_size=args.pop, generations=args.generations,
		...
		tour, dist = nearest_neighbor_multistart(coords, restarts=args.restarts, seed=args.seed)
