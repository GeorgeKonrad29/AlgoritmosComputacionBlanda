...
# Parámetros del CBGA - Optimizados para st70
params_cbga = {
    "pop_size": 100,
    "generations": 150,        # más generaciones = más tiempo de evolución
    "min_diversity": 0.25,     # menos restrictivo, permite más variedad
    "p_mut": 0.15,             # mutación moderada (10-15% swaps)
    "p_ls_child": 0.15,        # intensificación selectiva (10-15% de hijos)
    "seed": None,
    "apply_2opt_on_new": True, # activar 2-opt para refinar hijos prometedores
    "tournament_k": 3          # presión selectiva moderada
}
...
for seed in range(NUM_RUNS):
    ...
    # ---- Ejecución del CBGA ----
    start = time.perf_counter()
    _, dist_cbga, hist_cbga = cbga(coords, **params_cbga)
...
plt.legend()
plt.grid(True)
plt.savefig("figuras/convergencia_promedio.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True)
plt.savefig("figuras/estabilidad_soluciones.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True)
plt.savefig("figuras/costo_computacional.png", dpi=300, bbox_inches='tight')
plt.close()
